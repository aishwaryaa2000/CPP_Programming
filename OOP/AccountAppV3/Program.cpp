...
enum choice { createAcc=1, withdraw, deposit, displaySingleAcc,displayAllAcc,printRichest, printAllTransactions,exitApp };
...
int main()
{
    ...
    while (true) {
        cout << "\n\n1-Create an account" << endl
             ...
             << "7-Print transaction details of a particular account" << endl
             << "8-EXIT"<<endl
             << "Enter your choice : ";
        cin >> userInput;
        switch (userInput) {
        ...
        case printAllTransactions:
        {
            shared_ptr<Account> currentAcc = searchAccount();
            if (currentAcc) {
                currentAcc->PrintTransactionDetails();
            }
        }
            break;
        case exitApp:
